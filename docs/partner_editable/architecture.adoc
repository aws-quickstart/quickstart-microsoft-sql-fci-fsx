...
** In Availability Zone 1, an FSx file system that acts as a file-share (failover cluster) witness.
* The main FSx file system, which the FCI nodes share. SQL Server is installed in this file system. This file system also stores all SQL database-related files: the database files themselves, the database log files, and `tempdb` files.*
* AWS Directory Service with a managed directory. The main FSx file system and the EC2 Windows instances that host this architecture's nodes are joined to the same AD domain.
...
The SQL Server with Always On replication Quick Start, by default, does not use Amazon FSx. While it supports using an FSx file system as a file-share witness, by default it deploys an EC2 instance for this purpose.
...
That shared FSx file system also stores all the SQL database-related files: the database files themselves, database log files, and `tempdb` files. Unlike an availability group, an FCI does not replicate database-related files across the cluster. 

The {partner-product-short-name} architecture uses a separate Amazon FSx file system as a file-share witness.
